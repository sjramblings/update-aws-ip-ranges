...
def get_ranges_for_service(
    ranges: dict, config_services: dict
) -> dict[str, ServiceIPRange]:
    ...
    for config_service in config_services["Services"]:
        ...
        if len(config_service["Regions"]) > 0:
            ...
            logger.info(
                f"Will search for '{service_name}' without consider a region, so will get all prefixes from all regions"
            )
            key = f"{service_name}"
    ...
    for prefix in ranges["prefixes"]:
        ...
        if key in service_control:
            logger.info(
                f"Found service: '{service_name}' region: '{service_region}' range: {prefix['ip_prefix']}"
            )
            ...
            if key in service_control:
                logger.info(
                    f"Found service: '{service_name}' range: {prefix['ip_prefix']}"
                )
                service_ranges[service_name].ipv4.ip_list.append(prefix["ip_prefix"])
    ...
    for ipv6_prefix in ranges["ipv6_prefixes"]:
        ...
        if key in service_control:
            logger.info(
                f"Found service: '{service_name}' region: '{service_region}' range: {ipv6_prefix['ipv6_prefix']}"
            )
            ...
            if key in service_control:
                logger.info(
                    f"Found service: '{service_name}' range: {ipv6_prefix['ipv6_prefix']}"
                )
                service_ranges[service_name].ipv6.ip_list.append(
...
def manage_waf_ipset(
    ...
    for ip_version in ["ipv4", "ipv6"]:
        if not service_ranges[service_name].asdict()[ip_version].ip_list:
            logger.debug(
                f'No IP ranges found for service "{service_name}" and IP version "{ip_version}"'
            )
        else:
            # Found ranges for specific IP version (ipv4 or ipv6)
            address_list: list[str] = (
                service_ranges[service_name].asdict()[ip_version].ip_list
            )
            logger.debug(
                f'Summarize: "{should_summarize}" and address list lenght: "{len(address_list)}"'
            )
            if should_summarize and (len(address_list) > 1):
                address_list = (
                    service_ranges[service_name].asdict()[ip_version].summarized()
                )
            ...
            if ipset_name in waf_ipsets:
                ...
                updated: bool = update_waf_ipset(
                    client,
                    ipset_name,
                    ipset_scope,
                    waf_ipsets[ipset_name],
                    address_list,
                )
                if updated:
                ...
                create_waf_ipset(
                    client, ipset_name, ipset_scope, ip_version.upper(), address_list
                )
...
def create_waf_ipset(
    ...
    logger.info(
        f'Creating IPSet "{ipset_name}" with scope "{ipset_scope}" with {len(address_list)} CIDRs. List: {address_list}'
    )
...
def update_waf_ipset(
    ...
    network_list: list[Union[ipaddress.IPv4Network, ipaddress.IPv6Network]] = [
        ipaddress.ip_network(net) for net in address_list
    ]
    # Get current IPSet entries
    current_entries: dict[
        Union[ipaddress.IPv4Network, ipaddress.IPv6Network], str
    ] = get_ip_set_entries(client, ipset_name, ipset_scope, ipset_id)
    # Filter to get the list of entries to remove from IPSet
    entries_to_remove: list[str] = [
        cidr.with_prefixlen
        for cidr in current_entries.keys()
        if cidr not in network_list
    ]
    # Filter to get the list of entries to add into Prefix List
    entries_to_add: list[str] = [
        cidr.with_prefixlen for cidr in network_list if cidr not in current_entries
    ]
    ...
    if (not entries_to_add) and (not entries_to_remove):
        ...
        # Update IPSet
        logger.info(
            f'Updating IPSet "{ipset_name}" with scope "{ipset_scope}" with lock_token "{ipset_lock_token}" with {len(address_list)} CIDRs. List: {address_list}'
        )
        ...
        response = client.tag_resource(
            ...
            Tags=[
                {"Key": "UpdatedAt", "Value": datetime.now(timezone.utc).isoformat()}
            ],
        )
...
def list_waf_ipset(client: Any, ipset_scope: str) -> dict[str, dict]:
    ...
    while True:
        ...
        logger.info(
            f'Listed IPSet with scope "{ipset_scope}" and NextMarker "{next_marker}"'
        )
        logger.debug(f"Response: {response}")
...
def get_ip_set_entries(
    client: Any, ipset_name: str, ipset_scope: str, ipset_id: str
) -> dict[Union[ipaddress.IPv4Network, ipaddress.IPv6Network], str]:
    ...
    logger.info(
        f'Got IPSet with name "{ipset_name}" and scope "{ipset_scope}" and ID "{ipset_id}"'
    )
    logger.debug(f"Response: {response}")
...
def manage_prefix_list(
    ...
    for ip_version in ["ipv4", "ipv6"]:
        if not service_ranges[service_name].asdict()[ip_version].ip_list:
            ...
            logger.debug(
                f'Summarize: "{should_summarize}", Share: "{should_share}" and address list lenght: "{len(address_list)}"'
            )
            if should_summarize and (len(address_list) > 1):
                address_list = (
                    service_ranges[service_name].asdict()[ip_version].summarized()
            ...
            if prefix_list_name in vpc_prefix_lists:
                ...
                logger.debug(
                    f'VPC Prefix List "{prefix_list_name}" found. Will update it.'
                )
                updated: bool = update_prefix_list(
                    client,
                    prefix_list_name,
                    vpc_prefix_lists[prefix_list_name],
                    address_list,
                )
                if updated:
                ...
                logger.debug(
                    f'VPC Prefix List "{prefix_list_name}" not found. Will create it.'
                )
                reponse = create_prefix_list(
                    client, prefix_list_name, ip_version.upper(), address_list
                )
                ...
                if should_share:
                    ...
                    logger.debug(
                        f'VPC Prefix List "{prefix_list_name}" to be shared. Will create a RAM share.'
                    )

                    if "unset" in AWS_ORG_ARN:
                        logger.warning(
                            f"AWS Organization Id not found in AWS_ORG_ARN environment variable. Unable to proceed with sharing Prefix list."
                        )
                    else:
                        create_prefix_ram(
                            ram_client,
                            prefix_list_name,
                            reponse["PrefixList"]["PrefixListArn"],
                        )
...
def create_prefix_list(
    ...
    for addr in address_list:
        prefix_list_entries.append({"Cidr": addr, "Description": DESCRIPTION})
    ...
    logger.info(
        f'Creating VPC Prefix List "{prefix_list_name}" with max entries "{max_entries}" with address family "{prefix_list_ip_version}" with {len(address_list)} CIDRs. List: {address_list}'
    )
...
def update_prefix_list(
    ...
    current_entries: dict[
        ...
        client, prefix_list_name, prefix_list_id, prefix_list_version
    )
    # Filter to get the list of entries to remove from Prefix List
    entries_to_remove: list[dict] = [
        {"Cidr": cidr.with_prefixlen}
        for cidr in current_entries.keys()
        if cidr not in network_list
    ]
    # Filter to get the list of entries to add into Prefix List
    entries_to_add: list[dict] = [
        {"Cidr": cidr.with_prefixlen, "Description": DESCRIPTION}
        for cidr in network_list
        if cidr not in current_entries
    ]
    logger.debug(f"current_entries: {current_entries}")
    logger.debug(f"entries_to_remove: {entries_to_remove}")
    logger.debug(f"entries_to_add: {entries_to_add}")
    ...
    if (not entries_to_add) and (not entries_to_remove):
        ...
        if entries_to_add:
            ...
            if prefix_list_max_entries < len(address_list):
                logger.debug(
                    f'Current Prefix List max entries "{prefix_list_max_entries}" is lower than new range length "{len(address_list)}", so will change it to increase'
                )
                ...
                logger.info(
                    f'Updating VPC Prefix List "{prefix_list_name}" with id "{prefix_list_id}" with max entries "{prefix_list_max_entries}" with version "{prefix_list_version}"'
                )
                ...
                if current_state not in [
                    ...
                    raise Exception(
                        f'Error updating VPC Prefix List max entries. Invalid state. Expecting "modify-in-progress" or "modify-complete" or "modify-failed". Got "{current_state}". Name: "{prefix_list_name}" ID: "{prefix_list_id}" current version: "{prefix_list_version}" new max entries: "{prefix_list_max_entries}" StateMessage: "{current_state_message}"'
                    )
                if current_state == "modify-failed":
                    raise Exception(
                        f'Error updating VPC Prefix List max entries. State is "modify-failed". Name: "{prefix_list_name}" ID: "{prefix_list_id}" current version: "{prefix_list_version}" new max entries: "{prefix_list_max_entries}" StateMessage: "{current_state_message}"'
                    )
                if current_state == "modify-in-progress":
                    logger.info(
                        "Updating VPC Prefix List max entries is in progress. Will wait."
                    )
                    for count in range(5):
                        seconds_to_wait: int = count + (count + 1)
                        logger.info(f"Waiting {seconds_to_wait} seconds")
                        time.sleep(seconds_to_wait)
                        wait_prefix_list: dict[str, Any] = get_prefix_list_by_id(
                            client, prefix_list_id
                        )
                        ...
                        raise Exception(
                            "Error updating VPC Prefix List max entries. Can't wait anymore."
                        )

        # Update Prefix List entries
        logger.info(
            f'Updating VPC Prefix List "{prefix_list_name}" with id "{prefix_list_id}" with version "{prefix_list_version}" with {len(address_list)} CIDRs.'
        )
        logger.info(
            f'Updating VPC Prefix List "{prefix_list_name}" Entries to add: {entries_to_add}'
        )
        logger.info(
            f'Updating VPC Prefix List "{prefix_list_name}" Entries to remove: {entries_to_remove}'
        )
        logger.info(
            f'Updating VPC Prefix List "{prefix_list_name}" Full list of entries: {address_list}'
        )
        response = client.modify_managed_prefix_list(
            PrefixListId=prefix_list_id,
        ...
        logger.info(
            f'Updating Tags for "{prefix_list_name}" with ID "{prefix_list_id}"'
        )
...
def get_prefix_list_entries(
    client: Any, prefix_list_name: str, prefix_list_id: str, prefix_list_version: int
) -> dict[Union[ipaddress.IPv4Network, ipaddress.IPv6Network], str]:
    ...
    logger.info(
        f'Getting VPC Prefix List entries "{prefix_list_name}" with id "{prefix_list_id}" with version "{prefix_list_version}"'
    )
    response = client.get_managed_prefix_list_entries(
        PrefixListId=prefix_list_id, TargetVersion=prefix_list_version
    )
    ...
    for entrie in response["Entries"]:
        ...
        ] = ipaddress.ip_network(entrie["Cidr"])
...
def lambda_handler(event, context):
    ...
    try:
        ...
        ip_ranges: dict[str, Any] = json.loads(
        ...
        #     }
        # }
        service_ranges: dict[str, ServiceIPRange] = get_ranges_for_service(
            ip_ranges, config_services
        )
        ...
        for config_service in config_services["Services"]:
            ...
            if service_name not in service_ranges:
                logger.warning(
                    f'Service name "{service_name}" not found in service ranges variable. This condition should NEVER happens. Possible bug in the code. Please investigate.'
                )
            else:
                # Handle Prefix List
                if "PrefixList" not in config_service:
                    logger.info(
                        f'Service "{service_name}" not configured with "PrefixList"'
                    )
                else:
                    if not config_service["PrefixList"]["Enable"]:
                        logger.info(
                            f'Service "{service_name}" is configured with "PrefixList" but it is not enable'
                        )
                    else:
                        try:
                            ...
                            if not vpc_prefix_lists:
                                logger.info(
                                    "Will get the list of VPC Prefix List for the first time"
                                )
                            ...
                            resource_names["PrefixList"][
                                "created"
                            ] += prefix_list_names["created"]
                            resource_names["PrefixList"][
                                "updated"
                            ] += prefix_list_names["updated"]
                        except Exception as error:
                            logger.error(
                                "Error handling VPC Prefix List. It will not block the execution. It will continue to other services and resources."
                            )
                            logger.exception(error)
        ...
        for config_service in config_services["Services"]:
            ...
            if service_name not in service_ranges:
                ...
                if "WafIPSet" not in config_service:
                    logger.info(
                        f'Service "{service_name}" not configured with "WafIPSet"'
                    )
                else:
                    if not config_service["WafIPSet"]["Enable"]:
                        logger.info(
                            f'Service "{service_name}" is configured with "WafIPSet" but it is not enable'
                        )
                    else:
                        # Scope can be 'CLOUDFRONT' or 'REGIONAL'
                        for ipset_scope in config_service["WafIPSet"]["Scopes"]:
                            logger.info(
                                f'Service "{service_name}" WAF Scope "{ipset_scope}"'
                            )
                            try:
                                waf_ipsets: dict[str, dict] = {}
                                ...
                                if ipset_scope in waf_ipsets_by_scope:
                                    ...
                                    logger.info(
                                        f'Will get the list of WAF IPSets for the first time for scope "{ipset_scope}"'
                                    )
                                    waf_ipsets = list_waf_ipset(waf_client, ipset_scope)
                                    waf_ipsets_by_scope[ipset_scope] = waf_ipsets

                                should_summarize = config_service["WafIPSet"][
                                    "Summarize"
                                ]
                                ipset_names: dict[str, list[str]] = manage_waf_ipset(
                                    waf_client,
                                    waf_ipsets,
                                    service_name,
                                    ipset_scope,
                                ...
                                resource_names["WafIPSet"]["created"] += ipset_names[
                                    "created"
                                ]
                                resource_names["WafIPSet"]["updated"] += ipset_names[
                                    "updated"
                                ]
                            except Exception as error:
                                logger.error(
                                    "Error handling WAF IPSet. It will not block the execution. It will continue to other services and resources."
                                )
                                logger.exception(error)
